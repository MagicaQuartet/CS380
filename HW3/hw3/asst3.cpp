...
static RigTForm g_viewpointRbt = g_skyRbt;
static RigTForm g_sphereRbt = RigTForm(Cvec3(0, 0, 0));
static double g_arcballScale = getScreenToEyeScale((inv(g_viewpointRbt) * g_sphereRbt).getTranslation()[2], g_frustFovY, g_windowHeight);
...
static void drawStuff() {
	...
	if ((g_objectView == 0 && g_objectToggle != 0) || (g_objectView == 0 && g_objectToggle == 0 && g_skyMode == 0) || g_objectView + g_objectToggle == 3) {
		...
		MVM = rigTFormToMatrix(invEyeRbt * g_sphereRbt) * Matrix4::makeScale(Cvec3(g_arcballScale, g_arcballScale, g_arcballScale));
		NMVM = normalMatrix(MVM);
		sendModelViewNormalMatrix(curSS, MVM, NMVM);
  ...
  if (g_mouseClickDown) {
	  if (g_objectToggle == 0) {
		  if (g_objectView == 0) {
			  ...
			  g_viewpointRbt = g_skyRbt;
		  }
	  }
	  else {
		  if (g_objectToggle == g_objectView) {
			  m = transFact(m) * inv(linFact(m));
		}
		  RigTForm a = transFact(g_objectRbt[g_objectToggle - 1]) * linFact(g_viewpointRbt);
		g_objectRbt[g_objectToggle - 1] = a * m * inv(a) * g_objectRbt[g_objectToggle - 1]; // Simply right-multiply is WRONG
		if (g_objectToggle == g_objectView) {
			g_viewpointRbt = g_objectRbt[g_objectToggle - 1];
		}
		else {
			g_sphereRbt = g_objectRbt[g_objectToggle - 1];
		}
	  }
...
static void keyboard(const unsigned char key, const int x, const int y) {
  ...
  case 'f':
    ...
    break;
  case 'o':
	  g_objectToggle = (g_objectToggle + 1) % 3;
	  if (g_objectToggle == 0) {
		  cout << "Now controlling sky" << endl;
		  if (g_objectView != 0)
			  cout << "You cannot control sky camera now" << endl;
		  else {
			  if (g_skyMode == 0) {
	  ...
	  else if (g_objectToggle == 1) {
		  cout << "Now controlling cube 1" << endl;
		  if (g_objectView != 1) {
			  g_sphereRbt = g_objectRbt[0];
		  }
	  }
	  else {
		  cout << "Now controlling cube 2" << endl;
  ...
  case 'v':
	  ...
	  if (g_objectView == 0) {
		  ...
		  g_viewpointRbt = g_objectRbt[0];
		  cout << "Current viewpoint: cube 1" << endl;
		  if (g_objectToggle == 0) {
			  cout << "You cannot control sky camera now" << endl;
		  }
		  else if (g_objectView != g_objectToggle) {
			  g_sphereRbt = g_objectRbt[g_objectToggle - 1];
		  }
	  }
	  else {
		  g_viewpointRbt = g_objectRbt[1];
