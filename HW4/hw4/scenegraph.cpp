...
public:
  ...
  const RigTForm getAccumulatedRbt(int offsetFromStackTop = 0) {
	  const int len = rbtStack_.size();
	  RigTForm accRbt = RigTForm(Cvec3(0, 0, 0), Quat(1, 0, 0, 0));

	  for (int i = 0; i < len - offsetFromStackTop; i++) {
		  accRbt = accRbt * rbtStack_[i];
	  }
  ...
  virtual bool visit(SgTransformNode& node) {
	  ...
	  return (target_ == node) ? false : true;
  }
