...
template<typename T, int n>
inline Cvec<T, n> lerp(const Cvec<T, n>& a, const Cvec<T, n>& b, const Cvec<T, n>& c, const Cvec<T, n>& d, double t) {
	assert(t >= 0 && t <= 1);
	return b * pow(1 - t, 3) + ((c - a) / 6 + b) * 3 * pow(1 - t, 2) * t + ((b - d) / 6 + c) * 3 * (1 - t) * pow(t, 2) + c * pow(t, 3);
}
